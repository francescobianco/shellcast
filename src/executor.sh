...
shellcast_executor_run_local() {
  ...
  local shell_bin; shell_bin="${SHELL:-sh}"
  # shellcheck disable=SC2086
  "$shell_bin" "$script" $args > "$stdout_file" 2> "$stderr_file" || exit_code=$?
...
# Return the version of the user's default shell ($SHELL).
shellcast_executor_local_version_summary() {
  local shell_bin; shell_bin="${SHELL:-sh}"
  local shell_name; shell_name=$(basename "$shell_bin")
  local ver; ver=""
  ver=$("$shell_bin" --version 2>&1 | head -1 | grep -oE '[0-9]+\.[0-9]+(\.[0-9]+)?' | head -1)
  if [ -n "$ver" ]; then
    printf '%s %s (%s)' "$shell_name" "$ver" "$shell_bin"
  else
    printf '%s (%s)' "$shell_name" "$shell_bin"
  fi
}

# Run version-introspection inside each registered shell container + local + macos.
shellcast_executor_discover_versions() {
  ...
  # local entry
  local local_ver; local_ver=""
  local_ver=$(shellcast_executor_local_version_summary)
  printf '%-22s %-30s %s\n' "local" "(host)" "$local_ver"
  ...
  # macOS BSD userland image
  local macos_ver; macos_ver="${SHELLCAST_MACOS_VERSION:-catalina}"
  local macos_token; macos_token="macos:${macos_ver}@macos"
  local macos_tag; macos_tag="shellcast-macos-${macos_ver}"

  shellcast_executor_ensure_image "$macos_token" >/dev/null 2>&1 || {
    printf '%-22s %-30s %s\n' "macos:${macos_ver}" "$macos_tag" "[image build failed]"
    return 0
  }

  local macos_ver_line; macos_ver_line=""
  macos_ver_line=$(docker run --rm "$macos_tag" shell-version 2>/dev/null) || true
  if [ -z "$macos_ver_line" ]; then
    macos_ver_line="[version unavailable]"
  fi
  macos_ver_line=$(printf '%s' "$macos_ver_line" | head -1 | sed 's/^[[:space:]]*//' | sed 's/[[:space:]]*$//')

  printf '%-22s %-30s %s\n' "macos:${macos_ver}" "$macos_tag" "$macos_ver_line"
}
